...
define([], function() {
    ...
    /* 
       ...
       var buf = new vbo.VertexAttribute(gl, {
            numComponents: 3, 
            dataType: gl.FLOAT,
            data: coords
        });       
    ...
    vbo.Attribute.prototype.bind = function(gl, program, attribName) {
        ...
        if (location >= 0) {
            ...
            gl.vertexAttribPointer(
                location, this.numComponents, 
                this.dataType, false, 0, 0
            );
            // enable this array / buffer
            gl.enableVertexAttribArray(location);
        }
        else if (this.warnUnused)
            console.log('vertex attribute '+ attribName + 
               ' not used in vertex shader.'
            );
    };
    
    /*
     * unbind() method of Attribute objects 
     * unbinds the VBO to its respective shader variable
     */
    vbo.Attribute.prototype.unbind = function(gl, program, attribName) {
        // get location to which this attribute is bound 
        // in the currently active WebGL program
        var location = program.getAttribLocation(attribName);
        if (location >= 0) {
            // unbind the buffer object from the current vertex array 
            gl.bindBuffer(gl.ARRAY_BUFFER, null);
            // disable this array / buffer
    ...
    */
    ...
    /*
     * method unbind(): unbinds the index buffer
     */ 
    vbo.Indices.prototype.unbind = function(gl) {
