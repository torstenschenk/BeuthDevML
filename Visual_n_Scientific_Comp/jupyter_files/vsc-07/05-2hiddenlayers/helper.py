...
def plot_loss(stats,name='plot_loss'):
...
def plot_accuracy(stats,name='plot_acc'):
...
def plot_net_weights(net):
    W0 = net.W0
    W0 = W0.reshape(32, 32, 3, -1).transpose(3, 0, 1, 2)
    plt.imshow(visualize_grid_of_images(W0, padding=3).astype('uint8'))
    plt.gca().axis('off')
    plt.show()
